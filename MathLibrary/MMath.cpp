...
Matrix3 MMath::transpose(const Matrix3& m) {
	return Matrix3(m[0], m[3], m[6],
				m[1], m[4], m[7],
				m[2], m[5], m[8]);
}

float MMath::determinate(const Matrix3& m){
...
float MMath::determinate(const Matrix4& m) {
...
Matrix3 MMath::inverse(const Matrix3 &m) {
		Matrix3 inverseM;
		Matrix3 transposeM;
		Matrix3 adjointM;
		float determinate;
